...
onMounted(async () => {
  const response = await fetch(`https://api.lanyard.rest/v1/users/748539900793716887`);
  if (!response.ok) {
    return;
  }
...
<template>
  ...
  <div class="flex items-center gap-2 text-sm text-[var(--color-secondary)]">
    ...
    <div v-if="spotify">
      i'm currently listening to <a :href="`https://open.spotify.com/track/${spotify.track_id}`" target="_blank" class="font-black underline">{{ spotify.song }} - {{ spotify.artist }}</a>.
    </div>
    <div v-else>
      i'm not listening to anything right now.
    </div>
  </div>
  <div>emirhan (aka larei), 18 years-old. mathematics student, self-taught developer, <a href="https://www.youtube.com/watch?v=9sJUDx7iEJw" target="_blank" class="underline">open-source</a> enthusiast.
